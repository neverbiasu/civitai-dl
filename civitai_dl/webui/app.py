...
def create_app():
    ...
    with gr.Blocks(title=f"Civitai Downloader v{__version__}", theme=gr.themes.Soft()) as app:
        # 顶部标题和导航
        with gr.Row():
        ...
        with gr.Tabs() as tabs:
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        output_dir = gr.Textbox(
                            label="输出目录", 
                            value=config.get("output_dir", os.path.join(os.getcwd(), "downloads"))
                        )
            ...
            with gr.Tab("图像下载"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        image_version_id = gr.Number(label="版本ID (可选)", precision=0, minimum=1)
                    ...
                    with gr.Column(scale=2):
                        image_gallery = gr.Gallery(
                            label="图像预览", show_label=True, 
                            columns=3, rows=3, height=600
                        )
            ...
            with gr.Tab("下载队列"):
                ...
                with gr.Row():
                    pause_all_btn = gr.Button("全部暂停")
                    resume_all_btn = gr.Button("全部恢复")
                    cancel_all_btn = gr.Button("全部取消")
            ...
            with gr.Tab("设置"):
                ...
                with gr.Accordion("下载设置"):
                    default_output = gr.Textbox(
                        label="默认下载路径", 
                        value=config.get("output_dir", "./downloads")
                    )
                    ...
                    chunk_size = gr.Slider(
                        minimum=1024, maximum=1024*32, step=1024, 
                ...
                with gr.Accordion("路径设置"):
                    ...
                    image_template = gr.Textbox(
                        label="图像路径模板", 
                        value=config.get("image_path_template", "images/{model_id}/{image_id}")
                    )
                    gr.Markdown("""
                    **可用的模板变量:**
                ...
                with gr.Row():
                    ...
                    export_settings_btn = gr.Button("导出设置")
                    import_settings_btn = gr.Button("导入设置")
                    settings_status = gr.Textbox(label="设置状态", interactive=False)

        # 页脚信息
        with gr.Row():
            gr.Markdown("Civitai Downloader | [项目地址](https://github.com/path/to/civitai-downloader) | [问题反馈](https://github.com/path/to/civitai-downloader/issues)")
        ...
        def on_download(model_id, version_id, output_dir, with_images, image_limit):
            ...
            try:
                ...
                # 打印下载信息便于调试
                print(f"正在下载模型: {model_info.get('name')} - {target_version.get('name')}")
                ...
                if with_images and image_limit > 0:
                    ...
                    Thread(
                        ...
                        args=(api, download_engine, model_id, target_version.get("id"), image_limit, output_dir),
                        daemon=True
                    ).start()
                
                return f"开始下载: {model_info.get('name')} - {target_version.get('name')}", 0
            
            except Exception as e:
                return f"下载出错: {str(e)}", 0

        def download_model_images(api, downloader, model_id, version_id, limit, output_dir):
            """后台下载模型的示例图像"""
            try:
                # 获取版本图像
        ...
        def save_settings(api_key, proxy, theme, output_dir, concurrent, chunk_size, 
                        model_template, image_template, timeout, max_retries, verify_ssl):
            """保存应用设置"""
            try:
                # 转换主题选项
        ...
        def update_progress():
            ...
            while True:
                ...
                if active_task:
                    # 计算进度百分比
                    progress = (active_task.downloaded / active_task.total) * 100 if active_task.total else 0
        ...
        search_images_btn.click(
            fn=on_image_search,
            inputs=[image_model_id, image_version_id, nsfw_filter, gallery_option, image_limit_slider],
            outputs=[image_gallery, image_metadata]
        )
        
        # 添加图像选择事件处理
        image_gallery.select(
            fn=on_image_selected,
            inputs=None,
        ...
        save_settings_btn.click(
            fn=save_settings, 
            inputs=[api_key, proxy, theme, default_output, concurrent, chunk_size,
                    model_template, image_template, timeout, max_retries, verify_ssl], 
