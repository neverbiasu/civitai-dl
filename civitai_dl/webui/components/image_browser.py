...
class ImageDownloader:
    ...
    def search_images(self, model_id: int, version_id: Optional[int] = None, 
                   nsfw_filter: str = "排除NSFW", gallery: bool = False, 
        ...
        try:
            ...
            for img in images:
                ...
                if url:
                    # 提取元信息作为标题
                    meta = img.get("meta", {})
                    prompt = meta.get("prompt", "无提示词") if isinstance(meta, dict) else "无元数据"
                    # 截取长提示词
                    if len(prompt) > 100:
                        prompt = prompt[:97] + "..."
    ...
    def download_images(self, model_id: int, version_id: Optional[int] = None, 
                      gallery: bool = False, nsfw_filter: str = "排除NSFW", 
                      limit: int = 10) -> str:
        """
        下载模型的图像
        ...
        try:
            ...
            output_dir = os.path.join(self.config.get("output_dir", "./downloads"), "images", folder_name)
