...
import urllib.parse
import re
from typing import Callable, Optional, List, Dict, Any  # 确保导入所有需要的类型
...
class DownloadTask:
    ...
    def __init__(self, url: str, file_path=None, output_path=None, filename=None, use_range=True,
                 headers=None, verify=True, proxy=None, timeout=30):
        """
        初始化下载任务
        可以通过两种方式指定保存路径:
        ...
        if file_path:
            ...
            if output_path:
                 ...
                 self._file_path = filename_from_url # Save in current directory if no path specified
        ...
        self.filename = os.path.basename(self._file_path) # Update filename after ensuring extension
        ...
        self._completion_callbacks = [] # List to hold completion callbacks for this task
                ...
                if "Content-Disposition" in head_response.headers:
                    ...
                    if header_filename:
                        ...
                        if new_file_path != self.file_path:
                             ...
                             self.file_path = new_file_path # Update using the property setter
            ...
            try:
                with requests.get(
                    ...
                    if "Content-Disposition" in response.headers:
                        ...
                        header_filename = self._extract_filename_from_header(content_disposition)
                        if header_filename and mode == "wb":  # Only update filename if starting fresh
                            dir_path = os.path.dirname(self.file_path)
                            new_file_path = os.path.join(dir_path, header_filename)
                            if new_file_path != self.file_path:
                    ...
                    if content_length_header:
                        ...
                        if mode == "ab": # Range request
                            ...
                            if self.total_size is None: # If total size wasn't known from HEAD
                                self.total_size = content_length + self.downloaded_size
                                logger.debug(f"从Range GET请求推断文件总大小: {self.total_size} 字节")
                        elif self.total_size is None: # Full download, size not known from HEAD
                ...
                if http_err.response is not None and http_err.response.status_code == 416:
                    ...
                    try:
                        ...
                        head_response = requests.head(self.url, headers=self.headers, timeout=10, verify=self.verify, proxies=proxies, allow_redirects=True)
                        ...
                        if remote_total_size != -1 and existing_size >= remote_total_size:
                             ...
                             self.downloaded_size = existing_size # May be slightly larger, but ok
                             self.status = "completed"
    ...
    def _extract_filename_from_header(self, content_disposition: str) -> Optional[str]:
        ...
        for match in fname:
             ...
             if match[0].startswith("UTF-8''") or match[1].startswith("UTF-8''"):
                 encoded_name = match[0][len("UTF-8''"):] if match[0] else match[1][len("UTF-8''"):]
...
class DownloadEngine:
    ...
    def __init__(
        ...
        if retry_times != 3 or retry_delay != 5:
             logger.warning("retry_times 和 retry_delay 参数在 DownloadEngine 上已弃用。重试逻辑现在由 DownloadTask 处理。")
    ...
    def register_progress_callback(self, callback: Callable[[int, Optional[int]], None]):
    ...
    def _handle_task_progress(self, task: DownloadTask, downloaded: int, total: Optional[int]):
    ...
    def download(self, url: str,
        ...
        if final_file_path is None:
            effective_output_path = output_path if output_path is not None else self.output_dir
            effective_filename = filename if filename is not None else self.get_filename_from_url(url)
        ...
        future = self.executor.submit(task.start, progress_callback=combined_progress_callback)

        # Optional: Add future handling if needed (e.g., future.add_done_callback)
        # but the task's internal callbacks should handle most completion logic.
    ...
    def get_filename_from_url(self, url: str) -> str:
        ...
        try:
            ...
            if not base_filename or '.' not in base_filename or base_filename.endswith('/'):
            ...
            base_filename = re.sub(r'[\\/*?:"<>|]', "_", base_filename).strip()
            if not base_filename: # Handle cases where sanitization results in empty string
                 import hashlib
                 hash_obj = hashlib.md5(url.encode())
    ...
    def download_batch(self, urls: List[str], output_dir: Optional[str] = None,
                       progress_callback: Optional[Callable[[int, Optional[int]], None]] = None,
                       completion_callback: Optional[Callable[[DownloadTask], None]] = None) -> List[DownloadTask]:
        """批量下载多个URL"""
        tasks = []
    ...
    def wait_all(self, timeout: Optional[float] = None):
        ...
        futures = [
             f for f in self.executor._threads # Accessing private member, might break
