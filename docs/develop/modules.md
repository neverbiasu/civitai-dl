...
| 基础设施层   | 基础服务和外部系统集成 | API客户端、下载引擎、数据存储  |
...
| 业务逻辑 | 元数据处理 | 处理和组织模型与图像元数据           | API客户端、数据存储             |
...
| 数据存储  | `storage.path`          | `~/.civitai-downloader`   | 数据库和配置保存路径 |
...
| 数据存储      | 数据库访问错误   | 使用缓存继续，记录错误日志 | 服务重启时尝试修复数据库        |
